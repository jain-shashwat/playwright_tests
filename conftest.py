...
def tracing_fixture(request, browser):
    ...
    # Create a unique file name for the trace file
    trace_file_path = f"inputs_outputs/traces/trace_{request.node.name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.zip"

    # Stop tracing after each test and save the trace
    context.tracing.stop(path=trace_file_path)
    context.close()

    print(f"Trace saved at {trace_file_path}")